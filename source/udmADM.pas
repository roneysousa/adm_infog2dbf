...
begin
   ...
   begin
      ...
      try
           ...
           Write(M_ARQUIVO, '1');
           // Identificação por Extenso do Tipo de Operação
           ...
           // Prefixo da Agência: Número da Agência onde está cadastrado o convênio líder da empresa
           ...
           // Número da Conta Corrente: Número da conta onde está cadastrado o Convênio Líder da empresa
           Write(M_ARQUIVO, tbCedentesCED_NRCONTA.AsString);
           // Dígito Verificador - D.V. - da Conta Corrente da empresa
           ...
           begin
               ...
               While not (aCDS.Eof) do
                begin
                    ...
                    Write(M_ARQUIVO, '7');
                    // 02.7 002 a 003 9(002) Tipo de Inscrição da empresa
                    If (tbCedentesCED_TPINSCRICAO.AsString = 'F') Then
                    begin
                        Write(M_ARQUIVO, '01');
                        // 03.7 004 a 017 9(014) Número do CPF/CNPJ da empresa
                    ...
                    begin
                        ...
                        Write(M_ARQUIVO, tbCedentesCED_NRINSCRICAO.AsString);
                    End;
                    // 04.7 018 a 021 9(004) Prefixo da Agência
                    Write(M_ARQUIVO, tbCedentesCED_AGENCIA.AsString);
                    // Dígito Verificador - D.V. - do Prefixo da Agência.
                    Write(M_ARQUIVO, tbCedentesCED_DIGAGE.AsString);
                    // 06.7 023 a 030 9(008) Número da Conta Corrente da empresa 02
                    Write(M_ARQUIVO, tbCedentesCED_NRCONTA.AsString);
                    // 07.7 031 a 031 X(001) Dígito Verificador - D.V. - do Número da Conta Corrente da empresa
                    Write(M_ARQUIVO, tbCedentesCED_DIGAGE.AsString);
                    // 08.7 032 a 038 9(007) Número do Convênio de Cobrança da empresa 02
                    Write(M_ARQUIVO, M_NRCONV);
                    // 09.7 039 a 063 X(025) Número de Controle do Participante. 23
                    Write(M_ARQUIVO, uFuncoes.StrZero(aCDS.FieldByName('CREDITO').AsString,25));
                    // 10.7 064 a 080 9(017) Nosso-Número
                    ...
                    Write(M_ARQUIVO, '000000');
                    // 19.7 102 a 106 X(005) Tipo de Cobrança
                    Write(M_ARQUIVO, uFuncoes.Replicate(' ',5));
                    // 20.7 107 a 108 9(002) Carteira de Cobrança
                    Write(M_ARQUIVO, tbCedentesCED_CARTEIRA.AsString);
                    // 21.7 109 a 110 9(002) Comando
                    Write(M_ARQUIVO, '01');
                    // 22.7 111 a 120 X(010) Seu Número/Número Atribuído pela empresa
                    ...
                    // 24.7 127 a 139 9(011)v99 Valor do Título 20
                    ...
                    Observações:
                    As espécies 25-Dívida Ativa da União, 26-Dívida Ativa de Estado, 27-Dívida Ativa de Município
                    somente são admissíveis nas Carteiras 11 e 17, na modalidade Simples
                    }
                    Write(M_ARQUIVO, '12');
                    // 29.7 150 a 150 X(001) Aceite do Título:
                    If uFuncoes.Empty(tbCedentesCED_ACEITE.asString) Then
                        Write(M_ARQUIVO, 'N')
                    Else
                        Write(M_ARQUIVO, tbCedentesCED_ACEITE.asString);
                    // 30.7 151 a 156 9(006) Data de Emissão: Informe no formato “DDMMAA”
                    Write(M_ARQUIVO, M_DTATUA);
                    // 31.7 157 a 158 9(002) Primeira Instrução Codificada
                    { INSTRUÇÃO CODIFICADA
                    a) Para Comando 01 - Registro de Título (posição 109-110)
                    ...
                    Write(M_ARQUIVO, '06');
                    // 32.7 159 a 160 9(002) Segunda Instrução Codificada
                    Write(M_ARQUIVO, '06');
                    ...
                    // 34.7 174 a 179 9(006) Data Limite para Concessão de Desconto/Data de Operação do BBVendor:
                    Write(M_ARQUIVO, '000000');
                    // 35.7 180 a 192 9(011)v99 Valor do Desconto
                    Write(M_ARQUIVO, '0000000000000');
                    // 36.7 193 a 205 9(011)v99 Valor do IOF/Qtde de Moeda Variável 30
                    Write(M_ARQUIVO, '0000000000000');
                    // 37.7 206 a 218 9(011)v99 Valor do Abatimento
                    Write(M_ARQUIVO, '0000000000000');
                    // 38.7 219 a 220 9(002) Tipo de Inscrição do Sacado
                    Write(M_ARQUIVO, '02');
                    // 39.7 221 a 234 9(014) Número do CNPJ ou CPF do Sacado
                    ...
                    //
                    Write(M_ARQUIVO, '02');
                    // 9(014) Número do CNPJ ou CPF do Sacado
                    Write(M_ARQUIVO, M_NRCNPJ);
                    // 48.7 392 a 393 X(002) Número de Dias Para Protesto
                    Write(M_ARQUIVO, '06');
                    // 49.7 394 a 394 X(001) Complemento do Registro: “Brancos”
                    Write(M_ARQUIVO, uFuncoes.replicate(' ', 1));
                    // 50.7 395 a 400 9(006) Seqüencial de Registro
                    Writeln(M_ARQUIVO, uFuncoes.StrZero(InttoStr(M_QTLINH),6));
                    //
                    M_QTLINH := M_QTLINH + 1;
                    Next;
           ...
           //
           //M_QTLINH := M_QTLINH + 1;
           // 01.9 001 a 001 9(001) Identificação do Registro Trailer: “9”
           Write(M_ARQUIVO, '9');
           // 02.9 002 a 394 X(393) Complemento do Registro: “Brancos”
           Write(M_ARQUIVO, uFuncoes.Replicate(' ', 393));
           // 03.9 395 a 400 9(006) Número Seqüencial do Registro no Arquivo
           Writeln(M_ARQUIVO, uFuncoes.StrZero(InttoStr(M_QTLINH),6));
       Finally
           CloseFile(M_ARQUIVO);
           //CloseFile(M_ARQREM);
     ...
     try
         try
              ...
              begin
                   ...
                   begin
                        ...
                        If (M_VLPROMOCAO > 0) then
                          aMensagemDesconto := #13+'DESCONTO DE R$ '+FormatFloat('###,##0.#0', M_VLPROMOCAO)+' ATÉ O VENCIMENTO.';
                        //  
             ...
             begin
                  ...
                  begin
                       ...
                       begin
                            SetParam('INSTRUCOES1','MULTA..: '+ FormatFloat('##0.#0', M_PEMULT) +'%  :: DE R$:  '+FormatFloat('##0.#0', M_VLMULT) +' APOS : '+aDataVenc+#13+
                                         ...
                                         'SUJEITO A PROTESTO APÓS 3 DIAS DE ATRASO'+aMensagemDesconto);
                  ...
                  Else
                      SetParam('ENDCLIE', qryLocClientesCLI_ENCLIE.AsString+', Nº.'+qryLocClientesCLI_NUMERO.AsString+' - '+qryLocClientesCLI_BACLIE.AsString);
                  //
...
begin
  ...
  if FileExists(ListaNomeCliente) then
    ...
    ReWrite(ArqCliente); { cria um novo se não existir }
  try
